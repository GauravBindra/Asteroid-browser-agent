...
#   1. wait_for_form_load(nova) - A helper function that waits for the form to fully load before we start filling it. This uses a simple time
#   delay and Nova-ACT's natural language understanding to ensure the form is interactive.
#   2. fill_text_field(nova, label, value) - The core function that fills a single text field by:
#     - Finding a field with the specified label using Nova-ACT's natural language capabilities
#     - Entering the provided value
#     - Adding a small delay to ensure the field is properly filled
#     - Handling any exceptions that occur
#   3. fill_easy_form_text_fields(nova, form_data) - A higher-level function that:
#     - Maps field labels to form data keys
#     - Iterates through all the text fields in the easy form
#     - Calls fill_text_field() for each field
#     - Tracks success/failure and reports overall status
