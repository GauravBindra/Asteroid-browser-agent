...
def extract_result_code(nova):
    ...
    try:
        # Use Nova-ACT to find and extract the result code
        result_text = nova.act("find and extract any text containing 'ASTEROID'", return_action_result=True)
        
        if not result_text:
            logger.error("No result text found")
            return None
        
        # Extract ASTEROID_X code using regex
        match = re.search(r"ASTEROID_[01]", result_text)
        if match:
            result_code = match.group(0)
            logger.info(f"Found result code: {result_code}")
            return result_code
        else:
            logger.error(f"Result code not found in text: {result_text}")
            return None
        
    except Exception as e:
        logger.exception(f"Error extracting result code: {e}")
        return None
...
#   1. Form Navigation Functions:
#     - click_button(nova, label): A generic function to click buttons by their label
#     - submit_form(nova): Handles the complete form submission process by:
#         - Clicking the "Review" button
#       - Waiting for the review page to load
#       - Clicking the "Submit" button
#       - Waiting for submission to complete
#   2. Result Extraction:
#     - extract_result_code(nova): Uses Nova-ACT to find and extract the ASTEROID code
#     - Uses regex to specifically identify "ASTEROID_0" or "ASTEROID_1"
#     - Returns the extracted code or None if not found
#   3. Complete Workflow Integration:
#     - automate_easy_form(nova, form_data): Ties everything together into a single workflow
#     - Fills text fields, handles checkboxes, submits the form, and extracts the result
#     - Returns success status and the result code
#     - Determines success based on whether the result code is "ASTEROID_1"