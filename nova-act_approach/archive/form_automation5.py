...
def extract_result_code(nova):
    ...
    try:
        # Use more specific Nova-ACT instruction to look below the Submit button
        result_text = nova.act("look below the Submit button and find any text containing 'ASTEROID'", return_action_result=True)
        
        if not result_text:
            # Try an alternative approach if the first one fails
            logger.info("Trying alternative method to find result code")
            result_text = nova.act("find and extract any text that appears to be a code or result on this page", return_action_result=True)
