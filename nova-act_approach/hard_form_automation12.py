#!/usr/bin/env python3
"""
Improved hard form automation functions with semantic field detection.

This module incorporates field_exists() for semantic lookup of fields to handle
the case where fields in the JSON data may not exist in the actual form.
This makes the form filling more robust against discrepancies between the 
data model and actual form implementation.
...
7. fill_business_info() function with semantic field detection
"""
...
# Import schema for semantic field detection
from nova_act import BOOL_SCHEMA

# Core utility functions with semantic field detection

def field_exists(nova, label_or_placeholder, *, max_probe_scrolls=2):
    """
    Semantic lookup: return True if an input whose label OR placeholder
    includes the given text is visible in the current viewport.
    
    Args:
        nova: NovaAct instance
        label_or_placeholder: Text to look for in field labels or placeholders
        max_probe_scrolls: Number of times to scroll down looking for field (default: 2)
        
    Returns:
        bool: True if field exists in the current form section
    """
    logger = logging.getLogger("nova_form_automation")
    logger.info(f"Checking if field '{label_or_placeholder}' exists on the page")
    
    try:
        # First check without scrolling
        query = (
            f"Is there an input element in view whose label or placeholder "
            f"includes '{label_or_placeholder}'? Answer true or false."
        )
        result = nova.act(query, schema=BOOL_SCHEMA)
        if result.matches_schema and result.parsed_response:
            logger.info(f"Field '{label_or_placeholder}' found immediately")
            return True

        # If not found, try scrolling a few times to reveal more of the form
        for i in range(max_probe_scrolls):
            logger.info(f"Field not in view, scrolling down (attempt {i+1}/{max_probe_scrolls})")
            nova.act("Scroll down just enough to reveal more of this section")
            
            # Check again after scrolling
            result = nova.act(query, schema=BOOL_SCHEMA)
            if result.matches_schema and result.parsed_response:
                logger.info(f"Field '{label_or_placeholder}' found after scrolling")
                return True
        
        # If we get here, the field wasn't found even after scrolling
        logger.info(f"Field '{label_or_placeholder}' not found after {max_probe_scrolls} scroll attempts")
        return False
...
def select_dropdown_option(nova, label, value):
    ...
    if not field_exists(nova, label):
        logger.warning(f"Dropdown field '{label}' not found on page, skipping")
        return False
...
def fill_date_field(nova, label, value):
    ...
    # First check if the field exists
    if not field_exists(nova, label):
        logger.warning(f"Date field '{label}' not found on page, skipping")
        return False
...
# Enhanced text field fill function with semantic field detection
def fill_text_field_with_detection(nova, label, value, max_retries=3):
    """
    Fill a text field with semantic field detection to handle missing fields.
    
    Args:
        nova: NovaAct instance
        label: Field label to look for
        value: Value to enter
        max_retries: Maximum retry attempts for filling
        
    Returns:
        bool: True if successful or field not found
    """
    logger = logging.getLogger("nova_form_automation")
    
    # First check if the field exists
    if not field_exists(nova, label):
        logger.info(f"Text field '{label}' not found on page, skipping")
        return False
    
    logger.info(f"Filling text field '{label}' with value '{value}'")
    result = enhanced_fill_text_field(nova, label, value, max_retries=max_retries)
    return result
...
def fill_contact_details(nova, contact_data, force_navigation=False):
    ...
    try:
        ...
        if "firstName" in contact_data:
            if not fill_text_field_with_detection(nova, "First Name", contact_data["firstName"]):
                logger.warning("Failed to fill first name")
        
        # Fill in last name (text field) - using enhanced function
        if "lastName" in contact_data:
            if not fill_text_field_with_detection(nova, "Last Name", contact_data["lastName"]):
        ...
        if "phoneNumber" in contact_data:
            if not fill_text_field_with_detection(nova, "Phone Number", contact_data["phoneNumber"]):
        ...
        if "jointInsured" in contact_data:
            ...
            if field_exists(nova, "Joint Insured"):
                ...
                if contact_data["jointInsured"] and "jointInsuredPersonName" in contact_data:
                    if not fill_text_field_with_detection(nova, "Joint Insured Person Name", contact_data["jointInsuredPersonName"], max_retries=5):
                ...
                logger.info("Joint Insured checkbox not found, skipping")
        
        # Fill in number of years as landlord (text field) - using enhanced function
        if "numberOfYearsAsLandlord" in contact_data:
            if not fill_text_field_with_detection(nova, "Number of Years as Landlord", str(contact_data["numberOfYearsAsLandlord"])):
...
def fill_business_info(nova, business_data, force_navigation=True):
    """
    Fill the Business Info section of the hard form with semantic field detection.
    
    Args:
        nova: NovaAct instance
        business_data: Dictionary containing business section data
        force_navigation: Force navigation to Business Info tab
        
    Returns:
        bool: True if successful and proceeded to next section
    """
    logger = logging.getLogger("nova_form_automation")
    logger.info("Filling Business Info section with semantic field detection")
    
    try:
        # Navigate to the Business Info tab if needed
        if force_navigation:
            if not navigate_to_tab(nova, "Business Info"):
                logger.warning("Failed to navigate to Business Info tab")
                return False
        
        # Brief pause to ensure form is fully loaded
        time.sleep(1)
        
        # Fill in business name (text field) if it exists
        if "name" in business_data:
            if not fill_text_field_with_detection(nova, "Business Name", business_data["name"]):
                logger.warning("Failed to fill business name")
        
        # Fill in business type (dropdown) if it exists
        if "type" in business_data:
            if not select_dropdown_option(nova, "Business Type", business_data["type"]):
                logger.warning("Failed to select business type")
        
        # Fill in trade/description (text field) if it exists
        if "trade" in business_data:
            if not fill_text_field_with_detection(nova, "Trade", business_data["trade"]):
                logger.warning("Failed to fill trade information")
        
        # Fill in address fields if present and they exist
        if "address" in business_data:
            # Address line 1
            if "addressLine1" in business_data["address"]:
                # Try by label first
                if not fill_text_field_with_detection(nova, "Address Line 1", business_data["address"]["addressLine1"]):
                    # Then try by placeholder
                    logger.info("Trying to find Address Line 1 field by placeholder")
                    if field_exists(nova, "Address Line 1", max_probe_scrolls=3):
                        try:
                            address_command = f"Find the field with placeholder 'Address Line 1' and enter '{business_data['address']['addressLine1']}'"
                            nova.act(address_command)
                            logger.info("Successfully filled Address Line 1 by placeholder")
                        except Exception as addr_error:
                            logger.warning(f"Failed to fill Address Line 1 by placeholder: {addr_error}")
            
            # Address line 2
            if "addressLine2" in business_data["address"]:
                # Try by label first
                if not fill_text_field_with_detection(nova, "Address Line 2", business_data["address"]["addressLine2"]):
                    # Then try by placeholder
                    logger.info("Trying to find Address Line 2 field by placeholder")
                    if field_exists(nova, "Address Line 2", max_probe_scrolls=3):
                        try:
                            address_command = f"Find the field with placeholder 'Address Line 2' and enter '{business_data['address']['addressLine2']}'"
                            nova.act(address_command)
                            logger.info("Successfully filled Address Line 2 by placeholder")
                        except Exception as addr_error:
                            logger.warning(f"Failed to fill Address Line 2 by placeholder: {addr_error}")
            
            # Address line 3
            if "addressLine3" in business_data["address"]:
                # Try by label first
                if not fill_text_field_with_detection(nova, "Address Line 3", business_data["address"]["addressLine3"]):
                    # Then try by placeholder
                    logger.info("Trying to find Address Line 3 field by placeholder")
                    if field_exists(nova, "Address Line 3", max_probe_scrolls=3):
                        try:
                            address_command = f"Find the field with placeholder 'Address Line 3' and enter '{business_data['address']['addressLine3']}'"
                            nova.act(address_command)
                            logger.info("Successfully filled Address Line 3 by placeholder")
                        except Exception as addr_error:
                            logger.warning(f"Failed to fill Address Line 3 by placeholder: {addr_error}")
            
            # City
            if "city" in business_data["address"]:
                # Try by label first
                if not fill_text_field_with_detection(nova, "City", business_data["address"]["city"]):
                    # Then try by placeholder
                    logger.info("Trying to find City field by placeholder")
                    if field_exists(nova, "City", max_probe_scrolls=3):
                        try:
                            address_command = f"Find the field with placeholder 'City' and enter '{business_data['address']['city']}'"
                            nova.act(address_command)
                            logger.info("Successfully filled City by placeholder")
                        except Exception as addr_error:
                            logger.warning(f"Failed to fill City by placeholder: {addr_error}")
        
        # Handle ERN tax code exemption checkbox
        if "exemptFromERNCode" in business_data and field_exists(nova, "Exempt from ERN Code"):
            if not handle_checkbox(nova, "Exempt from ERN Code", business_data["exemptFromERNCode"]):
                logger.warning("Failed to handle ERN code exemption checkbox")
        
        # Fill in ERN tax code if not exempt
        if not business_data.get("exemptFromERNCode", False) and "ernTaxCode" in business_data:
            if not fill_text_field_with_detection(nova, "ERN Tax Code", business_data["ernTaxCode"]):
                logger.warning("Failed to fill ERN tax code")
        
        # Fill in website URL if present
        if "websiteUrl" in business_data:
            if not fill_text_field_with_detection(nova, "Website URL", business_data["websiteUrl"]):
                # Try to find by placeholder since this field might have different label
                logger.info("Trying to find Website URL field by placeholder or nearby text")
                for field_name in ["Website URL", "Website", "URL", "Web address"]:
                    if field_exists(nova, field_name):
                        try:
                            web_command = f"Find a field for website or URL and enter '{business_data['websiteUrl']}'"
                            nova.act(web_command)
                            logger.info(f"Successfully filled Website URL using field '{field_name}'")
                            break
                        except Exception as web_error:
                            logger.warning(f"Failed to fill Website URL: {web_error}")
        
        # Fill in business description if present
        if "description" in business_data:
            if not fill_text_field_with_detection(nova, "Description", business_data["description"]):
                logger.warning("Failed to fill business description")
        
        # Fill in years of experience if present
        if "yearsOfExperience" in business_data:
            for field_name in ["Years of Experience", "Years in Business", "Experience in Years"]:
                if field_exists(nova, field_name):
                    try:
                        if not fill_text_field_with_detection(nova, field_name, str(business_data["yearsOfExperience"])):
                            logger.warning(f"Failed to fill {field_name}")
                        else:
                            logger.info(f"Successfully filled {field_name}")
                            break
                    except Exception as yrs_error:
                        logger.warning(f"Failed to fill {field_name}: {yrs_error}")
        
        # Improved verification step that doesn't navigate away
        logger.info("Verifying all Business Info fields are filled correctly")
        verification_command = (
            "Check the current Business Info section of the form only. "
            "DO NOT click any navigation buttons or tabs. "
            "Verify that all required fields are filled correctly. "
            "If any field is empty or incorrect, report which fields need attention "
            "but do not navigate away from this section or click any buttons."
        ...
        # Click Next to proceed to the next section
        if not click_next_button(nova):
            logger.error("Failed to click Next button after filling Business Info")
            return False
        
        logger.info("Successfully completed Business Info section and moved to next section")
        return True
        
    except Exception as e:
        logger.exception(f"Error filling Business Info section: {e}")
        return False

# Main orchestrator function for hard form
...
def automate_hard_form(nova, form_data):
    ...
    """
    logger = logging.getLogger("nova_form_automation")
    logger.info("Starting hard form automation with semantic field detection")
    
    try:
        # First, fill in the contact details section
        if not fill_contact_details(nova, form_data.get("contact", {})):
            logger.error("Failed to complete contact details section")
            return False, None
        
        # Next, fill in the business info section
        if not fill_business_info(nova, form_data.get("business", {})):
            logger.error("Failed to complete business info section")
            return False, None
        
        # Future sections will be implemented according to our step-by-step plan
        
        logger.warning("Remaining form sections not yet implemented")
        return False, None
        
    except Exception as e:
        logger.exception(f"Error during hard form automation: {e}")
        return False, None