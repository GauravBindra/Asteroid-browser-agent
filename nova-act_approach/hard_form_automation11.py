...
7. fill_business_info() function with address fields handling
"""
...
def fill_business_info(nova, business_data, force_navigation=True):
    """
    Fill the Business Info section of the hard form.
    ...
    """
    logger = logging.getLogger("nova_form_automation")
    logger.info("Filling Business Info section")
    ...
    try:
        ...
        # Fill in business name (text field) - using enhanced function
        if "name" in business_data:
            if not enhanced_fill_text_field(nova, "Business Name", business_data["name"]):
                logger.warning("Failed to fill business name")
        
        # Fill in business type (dropdown)
        ...
        # Fill in trade/description (text field) - using enhanced function
        if "trade" in business_data:
            if not enhanced_fill_text_field(nova, "Trade", business_data["trade"]):
                logger.warning("Failed to fill trade information")
        
        # Fill in address fields if present
        if "address" in business_data:
            # Address line 1
            if "addressLine1" in business_data["address"]:
                if not enhanced_fill_text_field(nova, "Address Line 1", business_data["address"]["addressLine1"]):
                    # Try with placeholder instead of label since some fields might not have visible labels
                    try:
                        ...
                        logger.warning(f"Failed to fill Address Line 1: {addr_error}")
            
            # Address line 2
            if "addressLine2" in business_data["address"]:
                if not enhanced_fill_text_field(nova, "Address Line 2", business_data["address"]["addressLine2"]):
                    try:
                        ...
                        logger.warning(f"Failed to fill Address Line 2: {addr_error}")
            
            # Address line 3
            if "addressLine3" in business_data["address"]:
                if not enhanced_fill_text_field(nova, "Address Line 3", business_data["address"]["addressLine3"]):
                    try:
                        ...
                        logger.warning(f"Failed to fill Address Line 3: {addr_error}")
            
            # City
            if "city" in business_data["address"]:
                if not enhanced_fill_text_field(nova, "City", business_data["address"]["city"]):
                    try:
                        ...
                        logger.warning(f"Failed to fill City: {addr_error}")
        
        # Handle ERN tax code exemption checkbox
        if "exemptFromERNCode" in business_data:
        ...
        if not business_data.get("exemptFromERNCode", False) and "ernTaxCode" in business_data:
            if not enhanced_fill_text_field(nova, "ERN Tax Code", business_data["ernTaxCode"]):
                logger.warning("Failed to fill ERN tax code")
        
        # Fill in website URL if present
        if "websiteUrl" in business_data:
            if not enhanced_fill_text_field(nova, "Website URL", business_data["websiteUrl"]):
        ...
        if "description" in business_data:
            if not enhanced_fill_text_field(nova, "Description", business_data["description"]):
        ...
        if "yearsOfExperience" in business_data:
            if not enhanced_fill_text_field(nova, "Years of Experience", str(business_data["yearsOfExperience"])):
                # Try alternative labels
                try:
                    logger.info("Trying alternative labels for years of experience field")
                    yrs_command = f"Find a field about years of experience or years in business and enter '{business_data['yearsOfExperience']}'"
                    nova.act(yrs_command)
                except Exception as yrs_error:
                    logger.warning(f"Failed to fill years of experience: {yrs_error}")
        ...
        verification_command = (
            ...
            "DO NOT click any navigation buttons or tabs. "
            "Verify that all required fields are filled correctly: "
            "Business Name, Business Type, Trade, Address fields, "
            "ERN Tax Code exemption checkbox, and if not exempt, the ERN Tax Code. "
            "If any field is empty or incorrect, report which fields need attention "
            "but do not navigate away from this section or click any buttons."
        )
        nova.act(verification_command)
...
def automate_hard_form(nova, form_data):
    ...
    """
    logger = logging.getLogger("nova_form_automation")
    logger.info("Starting hard form automation with business info handling")
    ...
    try:
        ...
        # Future sections will be added here as they are implemented
